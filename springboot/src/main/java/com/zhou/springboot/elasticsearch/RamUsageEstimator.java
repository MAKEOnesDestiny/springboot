...
public class RamUsageEstimator {
     ...
     */
    public final static int NUM_BYTES_OBJECT_REF; //开启压缩后是4字节，否则是8字节
        ...
        if (Constants.JRE_IS_64BIT) {
            ...
            try {
                ...
                if (hotSpotBean != null) {
                    ...
                    try {
                        final Object vmOption = getVMOptionMethod.invoke(hotSpotBean, "UseCompressedOops"); //指针压缩
                    ...
                    try {
                        final Object vmOption = getVMOptionMethod.invoke(hotSpotBean, "ObjectAlignmentInBytes"); //字节对齐
    ...
    /**
     * Return the size of the provided {@link Long} object, returning 0 if it is cached by the JVM and its shallow size
     * otherwise.
     */
    ...
    /**
     * Estimates a "shallow" memory usage of the given object. For arrays, this will be the memory taken by array
     * storage (no subreferences will be followed). For objects, this will be the memory taken by the fields.
     * <p>
    ...
    /**
     * Returns the shallow instance size in bytes an instance of the given class would occupy. This works with all
     * conventional classes and primitive types, but not with arrays (the size then depends on the number of elements
    ...
    public static long shallowSizeOfInstance(Class<?> clazz) {
        ...
        return alignObjectSize(size); //对齐
    }
     ...
     * measured so far. <code>f</code> is the field being probed.
     *
    ...
    /**
     * Return the size of the provided array of {@link Accountable}s by summing up the shallow size of the array and the
     * {@link Accountable#ramBytesUsed() memory usage} reported by each {@link Accountable}.
     */
